...
const GROWT_RUNS_CAP = 20;

/**
 * @param {NS} ns
 * @param {serverTarget} string
 * @param {targetGrowth} float
 * @param {serverCoresNumber} number
 */
function getTargetRequiredGrowthRun(ns, serverTarget, targetGrowth, serverCoresNumber) {
	// Get number of grow runs to reach TARGET_GROWTH current state
	let targetRequiredGrowthRun = Math.ceil(ns.growthAnalyze(serverTarget, targetGrowth, serverCoresNumber));
	// Set default value on special cases
	targetRequiredGrowthRun = [Infinity, NaN].includes(targetRequiredGrowthRun) ? 1 : targetRequiredGrowthRun;

	// If the current number of runs required is too high, find a better value that get in the maximum runs cap
	while (targetRequiredGrowthRun > GROWT_RUNS_CAP) {
		targetRequiredGrowthRun = Math.ceil(ns.growthAnalyze(serverTarget, targetGrowth, serverCoresNumber));
		targetGrowth = Math.round((targetGrowth - 0.01) * 100) / 100;
	}
...
async function execOnscannedServer(
	...
	if (playerHackLevel >= serverRequiredHackLevel) {
		...
		serversList.every(async (serverTarget) => {
			...
			const targetRequiredGrowthRun = getTargetRequiredGrowthRun(ns, serverTarget, targetGrowth, serverCoresNumber);
